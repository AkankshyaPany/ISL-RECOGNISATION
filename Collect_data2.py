...
if not os.path.exists("data2/output/train"):
    os.makedirs("data2/output/train")
if not os.path.exists("data2/output/test"):
    os.makedirs("data2/output/test")
for i in range(3):
    if not os.path.exists("data2/output/train/" + str(i)):
        os.makedirs("data2/output/train/"+str(i))
    if not os.path.exists("data2/output/test/" + str(i)):
        os.makedirs("data2/output/test/"+str(i))

for i in string.ascii_uppercase:
    if not os.path.exists("data2/output/train/" + i):
        os.makedirs("data2/output/train/"+i)
    if not os.path.exists("data2/output/test/" + i):
        os.makedirs("data2/output/test/"+i)
'''
...
#    roi = cv2.resize(roi, (64, 64))
    ...
    #cv2.imshow("ernel",kernel)
    # define range of skin color in HSV   (later on)
    lower_skin = np.array([30,34,45], dtype=np.uint8)  #0,20,70   (20,40,50)-Good [0,40,30] ,[20,40,40]
    upper_skin = np.array([209,224,244], dtype=np.uint8) # 20,255,255     [43,255,254]  ,[218,231,250]
        
     #extract skin colur image    # later on
    mask = cv2.inRange(hsv, lower_skin, upper_skin) #hsv range walon ko ok baki black
    #cv2.imshow("mask",mask)
    #cv2.imshow("mask",mask)
        
    #extrapolate the hand to fill dark spots within  #later on
    mask = cv2.dilate(mask,kernel,iterations = 4) #mask no mask1
    
    #cv2.imshow("mask1",mask)
        
    #blur the image
    mask = cv2.GaussianBlur(mask,(5,5),100)    
    blur = cv2.GaussianBlur(gray,(5,5),600)#2
    
    cv2.imshow("Gausmask",blur)
    ...
    ret, test_image = cv2.threshold(blur, minValue, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU) #th3 #mask    cv2.THRESH_BINARY_INV+
    
    #ret, test_image = cv2.threshold(mask, minValue, 255,cv2.THRESH_OTSU)
